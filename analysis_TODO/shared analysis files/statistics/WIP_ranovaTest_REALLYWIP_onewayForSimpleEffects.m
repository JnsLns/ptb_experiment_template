...
nPts = 20;
f1Levels = 2; % factor 1 levels 

% This script should compare only two comditions (for simple main effects)
useCondA = [1];
useCondB = [2];
...
for curStep = 1:size(useDat{1},1)
    ...
    % Keep only the two conditions compared here
    curStepDat = curStepDat(:,[useCondA useCondB],:)
    ...
    % do the ANOVA
    try
    [ps{curStep} tbls{curStep}] = anovan(curStepDat,group,'random',[1],'model','interaction','varnames',{'pts','conditions'},'display','off');
    end
...
%     d_z.ref(curStep) = mean(mean([condA,ref2Only],2)-mean([dtrOnly,pair1Only],2)) / std(mean([fullPair2,ref2Only],2)-mean([dtrOnly,pair1Only],2));
