...
% input data
useDat = a.byPts.trj.alg.pos.avg;
warning('using algined data!')
...
%     %end            
    ...
    try
        ...
        %if ~isempty(lastwarn) && isempty(lastValidAnova)
        if isempty(lastValidAnova)
            lastValidAnova = curStep - 1;
        end
    end
...
figure(hTgtFig);
    ...
    dLinesTmp = findobj(curLineAx,'tag','dataLine');
    % find end of shortest line (where nans start)
%     firstNanInd = inf;
%     for lNum = 1:numel(dLinesTmp)
%         curLine = dLinesTmp(lNum);        
%         curFirstNanInd = find(isnan(get(curLine,'XData')),1,'first');        
%         if curFirstNanInd < firstNanInd
%             firstNanInd = curFirstNanInd;
%         end
%     end        
    yDatTmp = get(dLinesTmp(1),'YData');
    firstNanInd = lastValidAnova;
    if ~isempty(firstNanInd)
        yDatTmp = yDatTmp(1:firstNanInd-1);
    end
    if iscell(yDatTmp)
