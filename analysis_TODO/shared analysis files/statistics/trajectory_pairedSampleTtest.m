...
testDescription = 'exp7_fullPairB_RvsL';
...
for curTimestep = 1:minLen
    ...
    [h(curTimestep), p(curTimestep), ci(1:2,curTimestep) , stats{curTimestep}] = ttest(x1,x2,'alpha',useAlpha);        
    ...
    % Note: What about direction of difference scores? Should accord to t and d_z!
    % Compute mean difference score
    mean_diff(curTimestep) = mean(x1-x2);
    % compute margin of error of confidence intervals for mean difference score
    %(at confidence level 1-useAlpha)
    [~, ci_moe(curTimestep)] = confidence_intervals(x1-x2, 1-useAlpha);
