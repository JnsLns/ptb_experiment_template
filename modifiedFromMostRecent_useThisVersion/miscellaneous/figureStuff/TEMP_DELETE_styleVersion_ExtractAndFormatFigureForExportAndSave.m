...
newXLims = [-60 60]; % (A) for R vs L; for figSize for three plots in a row in portrait (ensures constant ratio x-units/figWidth)
...
newYLims = [0 250]; % for all plots against percentual movement time
XTickVals = [-50 0 50];
...
%YTickVals = 0:50:250; % can be left empty [] to use ticks from source axis;
...
yLbl = '';
disableXTickLabels = false;
disableYTickLabels = false;
...
hAllLineAxes = findobj(0,'type','axes');
