...
% IMPORTANT NOTE: This version samples under the null hypothesis (that is,
% from the overall mean across the compared conditions), not from the
% individual condition means as described by Rick Dale (2007) and as done
% in the standard version of this script. 
% This was implemented to show that (1) sampling under the null case
% converges to a result equivalent to a simple Bernoulli process (in other
% words: it is independent of the input data), which would render bootstrapping
% unnecessary and (2) that what Dale has done in the 2007 paper (as well
% as Bartolotti, apparently) was not sampling under the null case because
...
for curFile = 1:nFiles
    ...
    parfor curExp = 1:bs.nExps
        ...
        meanOvrPts_a = mean(cell2mat(cellCollapse(loaded.bootstrapInput.conditionA.avg,2,2)),2);
        meanOvrPts_b = mean(cell2mat(cellCollapse(loaded.bootstrapInput.conditionB.avg,2,2)),2);
        meanOvrPts = mean([meanOvrPts_a,meanOvrPts_b],2);                
        stdOvrPts = std(cell2mat([loaded.bootstrapInput.conditionA.avg,loaded.bootstrapInput.conditionB.avg]),0,2);
