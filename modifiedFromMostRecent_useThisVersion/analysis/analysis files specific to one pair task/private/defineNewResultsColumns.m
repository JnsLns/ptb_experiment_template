...
if ~isfield(a.s.resCols,'refSide')
    a.s.resCols.refSide = max(structfun(@(x) x,a.s.resCols))+1;
end
...
% the target:
if ~isfield(a.s.resCols,'refComSame')
    a.s.resCols.refComSame = max(structfun(@(x) x,a.s.resCols))+1;
end
if ~isfield(a.s.resCols,'refNciSame')
    a.s.resCols.refNciSame = max(structfun(@(x) x,a.s.resCols))+1;
end
if ~isfield(a.s.resCols,'comNciSame')
    a.s.resCols.comNciSame = max(structfun(@(x) x,a.s.resCols))+1;
end
...
if ~isfield(a.s.resCols,'refVsComVsNciSide')
    a.s.resCols.refVsComVsNciSide = max(structfun(@(x) x,a.s.resCols))+1; % For considering effects of reference side
end
if ~isfield(a.s.resCols,'comVsRefVsNciSide')
    a.s.resCols.comVsRefVsNciSide = max(structfun(@(x) x,a.s.resCols))+1; % For considering effects of COM side
end
if ~isfield(a.s.resCols,'nciVsRefVsComSide')
    a.s.resCols.nciVsRefVsComSide = max(structfun(@(x) x,a.s.resCols))+1; % For considering effects of NCI (non-chosen item in target color) side
end
...
% sideNonChosenToDirPath * refSide * comSide (see prepareResults for codes);
% should be used for balancing in analysis across all these factors (e.g.,
% for pure "kinematic" effect).
if ~isfield(a.s.resCols,'absSideCategoryNciRefCom')
    a.s.resCols.absSideCategoryNciRefCom = max(structfun(@(x) x,a.s.resCols))+1;
end
