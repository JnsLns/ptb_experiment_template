...
cell_3_lndx = sub2ind(size(testWhat),cell_3_subs(1),cell_3_subs(2),cell_3_subs(3));
tr_x3 = cell2mat(cellCollapseAndTrim(testWhat{cell_3_lndx},1,3));
cell_4_lndx = sub2ind(size(testWhat),cell_4_subs(1),cell_4_subs(2),cell_4_subs(3));
tr_x4 = cell2mat(cellCollapseAndTrim(testWhat{cell_4_lndx},1,3));

[h, p, d, isNonNormal_ks, isNonNormal_lf, isNonNormal_sw, isNonNormal_ad, varNonEqual, p_lev, vars] = deal([]);
...
for curTimestep = 1:minLen
    ...
    x2 = squeeze(tr_x2(curTimestep,x1Col,:));    
    x3 = squeeze(tr_x3(curTimestep,x1Col,:));    
    x4 = squeeze(tr_x4(curTimestep,x1Col,:));    
    allRawData = [x1;x2;x3;x4];
        
    %Use residuals? (computed within each group)
    r_x1 = x1-mean(x1);
    r_x2 = x2-mean(x2);
    r_x3 = x3-mean(x3);
    r_x4 = x4-mean(x4);        
    allResiduals = [r_x1;r_x2;r_x3;r_x4];

    % test normality of residuals across groups
    dataToTestForNorm = allResiduals;
        
    % compute variance just to check
    vars(curTimestep,1) = var(x1);
    vars(curTimestep,2) = var(x2);
    vars(curTimestep,3) = var(x3);
    vars(curTimestep,4) = var(x4);
...
%     end
    ...
    try
        ...
        normalityAlpha = 0.05;
        isNonNormal_ks(curTimestep) = kstest((dataToTestForNorm-mean(dataToTestForNorm))/std(dataToTestForNorm),'alpha',normalityAlpha); % 0 if normal, 1 if nonnormal
        isNonNormal_lf(curTimestep) = lillietest((dataToTestForNorm),'alpha',normalityAlpha); % 0 if normal, 1 if nonnormal
        isNonNormal_sw(curTimestep) = swtest((dataToTestForNorm),normalityAlpha); % 0 if normal, 1 if nonnormal
        isNonNormal_ad(curTimestep) = adtest((dataToTestForNorm),'alpha',normalityAlpha); % 0 if normal, 1 if nonnormal
        
        % test for equal variances (yellow dots)
        useplev = 0.05; %sig level for Levene's test
        n = size(x1,1);                
        % using Levenes test (this works!)
        data = [allRawData(:), [ones(1,n),2*ones(1,n),3*ones(1,n),4*ones(1,n)]'];
        p_lev(curTimestep) = Levenetestp(data,useplev):      
        varNonEqual(curTimestep) = (p_lev(curTimestep)<useplev);
...
disp(['non-homeoscedastic steps (p<', num2str(useplev),'): ',num2str(sum(varNonEqual))]);
...
if doFigureStuff
    ...
    for i = 1:numel(isNonNormal_ks)
        ...
        if ~isnan(isNonNormal_ad(i)) && isNonNormal_ad(i)
            ...
            hNNMarkers_ad = plot(sigAx.XLim(1)+nnMarkerPlacementProp*diff(sigAx.XLim),yValueForSigMarkers,'marker','.','color','c', 'MarkerSize',sigMarkerSize); % for warped trajs
            set(hNNMarkers_ad,'tag','nnswDot');
            set(get(get(hNNMarkers_ad,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');            
        end
        % EqualVariances (Levene)
        nnMarkerPlacementProp   = sigMarkerPlacementProp + sigMarkerPlacementProp*.5;
        if ~isnan(varNonEqual(i)) && varNonEqual(i)
            switch interpType
                case  'wrp'
                    yValueForSigMarkers = i;
                case 'alg'
            ...
            hEVMarkers = plot(sigAx.XLim(1)+nnMarkerPlacementProp*diff(sigAx.XLim),yValueForSigMarkers,'marker','.','color','y', 'MarkerSize',sigMarkerSize); % for warped trajs
            set(hEVMarkers,'tag','evDot');
            set(get(get(hEVMarkers,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');            
        end
    end
    hold(sigAx,'off');
    %---------------------------------------------------------------------
