...
% for effect size colormap
if strcmp('dz',useEsType)
    plotWhatEffect_es = d_z.(whichEff);
    esColormapLims = [0 4]; % for d_z
elseif strcmp('pe2',useEsType)
    plotWhatEffect_es = pe2.(whichEff);
    esColormapLims = [0 1];
else
    error('Erroneous effect size type defined');
end
...
figure(hTgtFig);
    ...
    ymin = min(min(yDatTmp)); % y extent
    ymax = max(max(yDatTmp)); % y extent
    hEsImg = image([esAx.XLim(2)-barWidth, esAx.XLim(2)],[ymin ymax],plotWhatEffect_es');
    hEsImg.Tag = 'esImg';
    set(hEsImg,'alphadata',~isnan(plotWhatEffect_es')) % make transparent where d has nans and where nonsig.
    colormap(esAx,hot);
    hEsImg.CDataMapping = 'scaled';
    set(esAx,'Clim',esColormapLims,'ydir','normal');
    esAx.XLim = curLineAx.XLim;
    esAx.Tag = 'esAx';
    esAx.Visible = 'off';
